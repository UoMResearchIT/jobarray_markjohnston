function [tr,tr_lst,mmov]=polyparticletracker_parallelx(filenaming,frameseries,settings,interactive)
% ----------------------------------
...
elseif ismember('avifile',fieldnames(filenaming))
    ...
    global videoreadvideo
    videoreadvideo = VideoReader(filenaming.avifile);
    nseq=1;
...
%     set(axeshandle,'YLim',[1 sizeroi(1)])
    set(axeshandle,'XLim',[1 512])
    set(axeshandle,'YLim',[1 512])
    hold on
    drawnow
    axis manual
end
...
for n=2:length(frameseries)
    ...
    % display progress
    if ismember(n,round(linspace(1,length(frameseries),10)))
        disp(['Processing frame ' num2str(n) ' of ' num2str(length(frameseries))])
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function frameb=readandfilter
        % select the right frame and put it in frameb
        if avifilereader
            videoreadvideo.CurrentTime = (n-1)./videoreadvideo.FrameRate;
            frameb = readFrame(videoreadvideo);
            %frameb=frame2im(avidata(n-n_nextseq(nseq)+1));
        else
            frameb=imread(filename);
        end
        % convert from heightxwidthx3 to heightxwidth
        if length(roi)==1
