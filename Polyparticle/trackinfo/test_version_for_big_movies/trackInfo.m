...
% a common
% prefix, followed by the frame number. It should be the same as used for generating the tracking data with PolyParticleTracker.
%
% FOR AVI FILES:
...
handles.stitchOpt.perform = 0; %Don't Stitch tracks by default
...
handles.minNoFrames = 1000; %
handles.FrameJump = 1000;
...
% Only select tracks that are longer 'than minNoFrames'
lengthOK = [];
for j = 1:length(tracks)
    if length(tracks{j}) >= handles.minNoFrames
        lengthOK = [lengthOK j];
    end
end
tempcell = {};
for k = 1:length(lengthOK)
    tempcell{k} = tracks{lengthOK(k)};
end
tracks = tempcell;
clear tempcell;

%To save memory, can resample the tracks with 'FrameJump'
if handles.FrameJump > 1
    for i = 1:length(tracks)
        tracks{i} = tracks{i}(1:handles.FrameJump:end,:);
    end
end

%Stitch tracks if box is ticked
if handles.stitchOpt.perform == 1
    tThresh=handles.stitchOpt.tThresh*str2double(get(handles.framerateedit,'String'))/handles.FrameJump;
...
haltTscale = handles.runhaltOpt.haltTscale*str2double(get(handles.framerateedit,'String'))/handles.FrameJump;
...
%Make the datacursor window a bit bigger
w = findObj('Tag','figpanel');
set(w,'Position',[945 1 200 120])
%
...
%Need to find which particle is numbered n
for i=1:length(handles.tr)
    if handles.tr{i}(1,4) == n
        n = i;
        break
    end
end
%Set the axis of the track subplot and insert length scale marker
axis(handles.trackAxes,[min(handles.tr{n}(:,1))-10 max(handles.tr{n}(:,1))+10 ...
    min(handles.tr{n}(:,2))-10 max(handles.tr{n}(:,2))+10])
rectangle('Position',[min(handles.tr{n}(:,1))-9,min(handles.tr{n}(:,2))-9,pixel_size,1]...
    ,'facecolor','w','Parent',handles.trackAxes)
text(double(min(handles.tr{n}(:,1))-6),double(min(handles.tr{n}(:,2))-6)...
...
if get(handles.holdtracks,'Value') == 1
    ...
    Lpix = handles.runhaltOpt.Lpix*str2double(get(handles.pixelsizeedit,'String'));
    haltTscale = handles.runhaltOpt.haltTscale*str2double(get(handles.framerateedit,'String'))/handles.FrameJump;
    [nRuns,runToggle,nHalts,haltToggle]=runhalt2(handles.position{n},Lpix,haltTscale);
    for N=1:nHalts
...
if get(handles.distspeedcheck,'Value') == 1
    dot = (frame_no-get(handles.trackSlider,'Min'))/handles.FrameJump+1;
...
pixel_size = str2double(get(handles.pixelsizeedit,'String'));
frame_rate = str2double(get(handles.framerateedit,'String'))/handles.FrameJump;
...
txt = {nam,...
    tag,...
    ['Length: ' num2str(round((handles.totalDisplacement(n)/pixel_size)*100)/100) 'microns'],...
    ['Lifetime: ' num2str(round((handles.lifetime(n)/frame_rate)/handles.FrameJump*100)/100) 's'],...   %lifetime and speed are calculated using frame numbers, so I need to recorrect for the FrameJump parameter used previous
    ['avSpeed: ' num2str(round(handles.meanSpeed(n)*(frame_rate/pixel_size)*handles.FrameJump*100)/100) 'microns/s']};

if isfield(handles,'direction')
    txt = [txt,['Angle: ' num2str(handles.direction.angle(n)) ' degrees']];
end
...
Lpix = handles.runhaltOpt.Lpix*str2double(get(handles.pixelsizeedit,'String'));
haltTscale = handles.runhaltOpt.haltTscale*frame_rate;
[nRuns,runToggle,nHalts,haltToggle]=runhalt2(handles.position{n},Lpix,haltTscale);
for N=1:nHalts
    indices2=find(haltToggle==N);
    plot(handles.trackAxes,handles.tr{n}(indices2,1),handles.tr{n}(indices2,2),'g','linewidth',2);
...
set(handles.trackSlider,'Sliderstep',[handles.FrameJump/(handles.tr{n}(end,3) - handles.tr{n}(1,3)+1) (10*handles.FrameJump)/(handles.tr{n}(end,3) - handles.tr{n}(1,3)+1)]);
...
if get(handles.distspeedcheck,'Value') == 1
    plotDistSpeed_runhalt(handles,handles.nam,1,1);
    set(handles.speedSave,'Visible','On')
    set(handles.speedSaveTxt,'Visible','On')
else
    cla(handles.distancePlot)
    set(handles.distancePlot,'Visible','Off')
    cla(handles.speedPlot)
    set(handles.speedPlot,'Visible','Off')
    set(handles.windowSlider,'Visible','Off')
...
% Speed and distance plots
%Calculate distance and speed
initial_x = handles.tracksmooth{n}(1,1);
initial_y = handles.tracksmooth{n}(1,2);
% % % Reset positions
tr1 = [];
tr1(:,1) = handles.tracksmooth{n}(:,1) - initial_x;
tr1(:,2) = handles.tracksmooth{n}(:,2) - initial_y;
%Interpolate stitches
[tr1] = interpolate_stitches(tr1);
%Rescale to microns
tr1(:,1:2) = tr1(:,1:2)/pixel_size;
