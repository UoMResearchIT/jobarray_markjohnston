...
Lpix=handles.positionAveragingOpt.Lpix*str2double(get(handles.pixelsizeedit,'String'));
% for n=1:length(handles.tr)
    [handles.position{n},handles.contour{n},handles.tracksmooth{n}]=positionAveraging(track,Lpix);
% end
% 
displacementThresh = handles.track_analysisOpt.displacementThresh*str2double(get(handles.pixelsizeedit,'String'));
tempcell = {track};
position = {handles.posistion{n}};
[meanSpeed,totalDisplacement,lifetime,indices] = track_analysis_dak3(tempcell,position,displacementThresh);


haltTscale = handles.runhaltOpt.haltTscale*str2double(get(handles.framerateedit,'String'));

   [nRuns,~,nHalts,~]=runhalt2(handles.position{n},Lpix,haltTscale);

txt = {nam,...
    tag,...
    ['nRuns:' num2str(nRuns)...
    ' nHalts:' num2str(nHalts)],...
    ['Length: ' num2str(round((totalDisplacement/pixel_size)*100)/100) 'microns'],...
    ['Lifetime: ' num2str(round((lifetime/frame_rate)*100)/100) 's'],...
    ['avSpeed: ' num2str(round(meanSpeed*(frame_rate/pixel_size)*100)/100) 'microns/s']};
...
set(handles.trackAxes,'NextPlot','Add')
%Set the axis of the track subplot and insert length scale marker
axis(handles.trackAxes,[min(track(:,1))-10 max(track(:,1))+10 ...
    min(track(:,2))-10 max(track(:,2))+10])
rectangle('Position',[min(track(:,1))-9,min(track(:,2))-9,pixel_size,1]...
    ,'facecolor','w','Parent',handles.trackAxes)
text(double(min(track(:,1))-6),double(min(track(:,2))-6)...
    ,'1{\mu}m','fontsize',14,'color','w','Parent',handles.trackAxes);


%Mark the start and end positions of each run
plot(handles.trackAxes,track(1,1),track(1,2),'yo','markersize',10,'linewidth',2);
text(double(track(1,1)+3), double(track(1,2)+3), ...
    'Start','fontsize',14,'color','w','Parent',handles.trackAxes);
plot(handles.trackAxes,track(end,1),track(end,2),'y+','markersize',10,'linewidth',2);
text(double(track(end,1)+3),double(track(end,2)+3),...
    'End','fontsize',14,'color','w','Parent',handles.trackAxes);
...
for N=1:nHalts
    indices2=find(haltToggle==N);
    plot(handles.trackAxes,track(indices2,1),track(indices2,2),'g','linewidth',2);
end
for N=1:nRuns
    indices2=find(runToggle==N);
    plot(handles.trackAxes,track(indices2,1),track(indices2,2),'m','linewidth',2);
end
...
set(handles.trackSlider,'Value',track(1,3))
set(handles.trackSlider,'Min',track(1,3));
set(handles.trackSlider,'Max',track(end,3));
set(handles.trackSlider,'Sliderstep',[1/(track(end,3) - track(1,3)+1) 10/(track(end,3) - track(1,3)+1)]);
set(handles.trackSlider,'Visible','On')
%Set the values to be displayed
set(handles.sliderMin,'String',['Min:' num2str(track(1,3))])
set(handles.sliderMax,'String',['Max:' num2str(track(end,3))])
...
set(handles.windowSlider,'Max',track(end,3)-track(1,3)+1);
